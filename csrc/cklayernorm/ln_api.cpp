...
at::Tensor ln_fwd(const at::Tensor &x,      // BxSxhidden_size
    ...
    Launch_params<lnck_fprop_params> launch_params(dprops, stream); 
    ...
    lnck_fprop_params &params = launch_params.params;
    params.rows = rows;
    params.cols = cols;
    params.x_ptr = reinterpret_cast<void*>(x.data_ptr());
    //params.mu = mu.data_ptr();
    //params.rs = rsigma.data_ptr();
    params.gamma_ptr = reinterpret_cast<void*>(gamma.data_ptr());
    params.beta_ptr = reinterpret_cast<void*>(beta.data_ptr());
    params.z_ptr = reinterpret_cast<void*>(z.data_ptr());
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("oldmain", &oldmain, "Run oldmain");
}
